...
export default function ColoringPage({ drawingKey, onBack }) {
  ...
  // Garde en mémoire la dernière zone cliquée (utile pour la gomme)
  ...
  const handleZone = (zone, e) => {
    ...
    // Effet étoile sur clic (sécurisé avec optional chaining)
    if (e?.target) {
    ...
    // Vérifie si terminé
    setTimeout(() => {
      if (isComplete({ ...colors, [zone]: selected }, zones)) {
        setDone(true);
      }
    }, 400);
  };

  // Gomme : remet en blanc la dernière zone cliquée
  const handleErase = () => {
  ...
  return (
    <div className="flex flex-col items-center pt-6">
      <div className="mb-2 text-xl font-semibold text-violet-700">
        {drawing.name}
      </div>
      <div
        className="relative bg-white rounded shadow p-4 mb-2"
        style={{ width: 320, height: 320 }}
      >
        <SVGComponent
          colors={colors}
          onZoneClick={(zone, e) => handleZone(zone, e)}
          id="coloring-svg"
        />
          ...
          <div className="absolute inset-0 bg-white bg-opacity-70 flex flex-col items-center justify-center z-30">
            ...
            <div className="flex items-center">
              <svg width="60" height="60">
                <polygon
                  points="30,5 38,27 55,27 41,38 47,55 30,45 13,55 19,38 5,27 22,27"
                  fill="#FFD700"
                  stroke="#FFC400"
                  strokeWidth="3"
                />
